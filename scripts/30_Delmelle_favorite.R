...
mets <- bball %>% filter(NAME == "Citi Field")
...
##Make the map
mets_map <- tm_shape(cont_us_poly)+
  tm_polygons(fill = "min", style = "pretty", n = 10, 
              title = "Hours", legend.is.portrait = FALSE))+
  tm_layout(frame = FALSE, legend.frame = FALSE)+
  tm_borders(col = NA)

mets_map <- mets_map + tm_shape(mets)+
  tm_symbols(fill = "brown1", col = "brown1", size = 1)

mets_map <- mets_map + 
  tm_title("My Favorite Team is the New York Mets\nThis is how long you'd have to travel to get to Citi Field")
